...
export default async function handler(req, res) {
  ...
  try {
    // Verify token (optional - allow both authenticated and unauthenticated requests)
    ...
    if (token) {
      ...
      try {
        ...
        // Token verification failed, but continue without userId for public access
    ...
    // generate shortUrl that points to the short code (/:shortCode)
    const shortUrl = base ? `${base.replace(/\/$/, "")}/${shortCode}` : `/${shortCode}`;

    const newUrl = await Url.create({
      userId: userId || undefined, // Allow anonymous links
      shortCode,
